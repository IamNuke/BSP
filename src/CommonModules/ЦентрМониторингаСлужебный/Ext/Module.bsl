...
Функция СформироватьСтруктуруJSONДляОтправки(Параметры)
	...
	JSONСтруктура.Вставить("versionPacket",  "1.0.0.0");
...
Процедура СформироватьПакетДляОтправки()
	...
	Параметры = Новый Структура;
	Параметры.Вставить("ДатаНачала", ПараметрыЦентраМониторинга.ДатаКрайнегоПакета + 1);
	Параметры.Вставить("ДатаОкончания", ТекДата);
...
Функция ВыполнятьЗамерыПроизводительности()
	...
	Если ОценкаПроизводительностиСуществует Тогда
		...
		ВыполнятьЗамерыПроизводительности = МодульОценкаПроизводительностиВызовСервераПолныеПрава.ВыполнятьЗамерыПроизводительности();
	Иначе
...
Функция ПолучитьПараметрыПоУмолчанию()
	...
	// ОценкаПроизводительностиВключена = 0 - Выключена
	// ОценкаПроизводительностиВключена = 1 - Включена центром мониторинга
	// ОценкаПроизводительностиВключена = 2 - Включена оценкой производительности
...
Функция УстановитьПараметрыОтправки(Параметры)
	...
	Попытка
		Если ПараметрыОтправки.ОценкаПроизводительностиВключена = 0 И НЕ ПараметрыОтправки.ВыполнятьЗамерыПроизводительности = Неопределено Тогда
			МодульОценкаПроизводительностиВызовСервераПолныеПрава = ОбщегоНазначения.ОбщийМодуль("ОценкаПроизводительностиВызовСервераПолныеПрава");
			МодульОценкаПроизводительностиВызовСервераПолныеПрава.ВключитьЗамерыПроизводительности(Ложь);
		ИначеЕсли ПараметрыОтправки.ОценкаПроизводительностиВключена = 1 И НЕ ПараметрыОтправки.ВыполнятьЗамерыПроизводительности = Неопределено Тогда
			МодульОценкаПроизводительностиВызовСервераПолныеПрава = ОбщегоНазначения.ОбщийМодуль("ОценкаПроизводительностиВызовСервераПолныеПрава");	
			МодульОценкаПроизводительностиВызовСервераПолныеПрава.ВключитьЗамерыПроизводительности(Истина);
		КонецЕсли;
...
Функция СоздатьФайлНастроекСбораДампов(ПутьКаталога)
	...
	|	<dump location=""" + КаталогДампов + """ create=""1"" type=""0""/>
	|</config>";
