...
Функция РеквизитыЮридическогоЛицаПоИНН(Знач ИНН) Экспорт
	...
	ЗаполнитьЮридическийАдрес(ОбъектXDTO, РеквизитыОрганизации);
...
//                 * РеквизитыЮрЛиц - Массив - реквизиты найденных юр.лиц.
...
Функция ПолучитьДанныеОРуководителеИНомерТелефона(СведенияОДолжности, Данные)
	...
	Если СтрНайти(СведенияОДолжности.НаимВидДолжн, "Руководитель") = 0 
	...
	Иначе
		...
		Попытка
			Прокси = ОбщегоНазначения.WSПрокси(
				...
				"RequisitesWebServiceEndpointImpl6Service", // ИмяСервиса
				"RequisitesWebServiceEndpointImpl6Port",    // ИмяТочкиПодключения
...
Функция АдресСервиса()
	...
	Возврат "https://api.orgregister.1c.ru/orgregister/v6?wsdl";
