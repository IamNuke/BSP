	...
	Для Каждого ТекущееПКО Из ИспользованиеПКО Цикл
		...
		Исключение
			ТекстОписаниеОшибки = "" + ОписаниеОшибки()
				+ Символы.ПС + НСтр("ru = 'Объект'") + ": " + СокрЛП(Объект);
				
			Попытка
				ТекстОписаниеОшибки = ТекстОписаниеОшибки
					+ Символы.ПС + НСтр("ru = 'Ссылка на объект'") + ": " + ПолучитьНавигационнуюСсылку(Объект.Ссылка);
			Исключение
				ТекстОписаниеОшибки = ТекстОписаниеОшибки + Символы.ПС + НСтр("ru = 'Ссылка на объект не доступна'");
			КонецПопытки;
...
Функция ДанныеXDTOИзДанныхИБ(КомпонентыОбмена, Источник, Знач ПравилоКонвертации, СтекВыгрузки = Неопределено) Экспорт
	...
	Если ПравилоКонвертации.ЭтоСсылочныйТип Тогда
		...
		Если СтекВыгрузки.Найти(Источник.Ссылка) <> Неопределено Тогда
...
//           "КонвертироватьВПолученныеДанные" - заполнение объекта только данными, полученными в результате
//                                               конвертации данных XDTO "КонвертироватьИЗаписать" - получение объекта,
//                                               готового.
//
// Возвращаемое значение:
...
Функция СтруктураОбъектаXDTOВДанныеИБ(КомпонентыОбмена, ДанныеXDTO, ПравилоКонвертации, Действие = "КонвертироватьИЗаписать") Экспорт
	...
	Если ПравилоКонвертации.ЭтоСсылочныйТип Тогда
		...
		Если ВариантИдентификации = "ПоУникальномуИдентификатору"
			Или (ВариантИдентификации = "СначалаПоУникальномуИдентификаторуПотомПоПолямПоиска"
				И ДанныеXDTO.Свойство("Ссылка")) Тогда
	...
	Если ДанныеИБ = Неопределено Тогда
		...
		ИначеЕсли ВариантИдентификации = "ПоПолямПоиска"
			...
			ДанныеИБ = СсылкаОбъектаПоСвойствамОбъектаXDTO(ПравилоКонвертации, ПолученныеДанные);
	...
	Если ЗаписыватьОбъектВИБ Тогда
		...
		Если КомпонентыОбмена.ЭтоОбменЧерезПланОбмена
			...
			И ВариантИдентификации = "СначалаПоУникальномуИдентификаторуПотомПоПолямПоиска"
			И ПравилоКонвертации.ЭтоСсылочныйТип Тогда
...
Процедура ПроизвестиЧтениеДанных(КомпонентыОбмена, ТаблицыДляЗагрузки = Неопределено) Экспорт
	...
	Если КомпонентыОбмена.ЭтоОбменЧерезПланОбмена И Не ЗначениеЗаполнено(КомпонентыОбмена.УзелКорреспондентаОбъект) Тогда
			...
			Для Каждого ТекущееПКО Из ИспользованиеПКО Цикл
				...
				Если ТекущееПКО.Значение Тогда
					...
					Если ВариантИдентификации = "ПоУникальномуИдентификатору"
						Или (ВариантИдентификации = "СначалаПоУникальномуИдентификаторуПотомПоПолямПоиска"
							И ДанныеXDTO.Свойство("Ссылка")) Тогда
						ПолученныеДанныеСсылка = ПравилоКонвертации.МенеджерОбъекта.ПолучитьСсылку(Новый УникальныйИдентификатор(ДанныеXDTO.Ссылка.Значение));
					КонецЕсли;
				...
				ИначеЕсли (ПравилоКонвертации.ВариантИдентификации = "СначалаПоУникальномуИдентификаторуПотомПоПолямПоиска"
							И ДанныеXDTO.Свойство("Ссылка"))
					Или ПравилоКонвертации.ВариантИдентификации = "ПоУникальномуИдентификатору" Тогда
					МассивОбъектовКУдалению.Добавить(СсылкаОбъектаПоУИДОбъектаXDTO(ДанныеXDTO.Ссылка.Значение, ПравилоКонвертации.ТипДанных, КомпонентыОбмена));
				КонецЕсли;
		...
		Иначе
			...
			Для Каждого ТекущееПКО Из ИспользованиеПКО Цикл
				...
				Если ТекущееПКО.Значение Тогда
					...
					СтрокаТаблицы.СинхронизироватьПоИдентификатору  = (ПравилоКонвертации.ВариантИдентификации = "СначалаПоУникальномуИдентификаторуПотомПоПолямПоиска"
						И ДанныеXDTO.Свойство("Ссылка")) Или ПравилоКонвертации.ВариантИдентификации = "ПоУникальномуИдентификатору";
...
Функция СсылкаОбъектаПоУИДОбъектаXDTO(УИДОбъектаXDTO, ТипЗначенияОбъектаИБ, КомпонентыОбмена) Экспорт
	...
	Если Не СсылкаПоУИД.Пустая() И ОбщегоНазначения.СсылкаСуществует(СсылкаПоУИД) Тогда
		Возврат СсылкаПоУИД; // Найден объект по уникальному идентификатору ссылки.
	КонецЕсли;
	...
	Если ПубличнаяСсылка <> Неопределено Тогда
		// Переданная ссылка не является публичной. Возврат "битой" ссылки.
...
Процедура ЗаписатьОбъектВИБ(КомпонентыОбмена, Объект, Тип, ЗаписатьОбъект = Ложь, Знач ОтправкаНазад = Ложь, УникальныйИдентификаторСтрокой = "") Экспорт
	...
	Исключение
		...
		ЗаписатьИнформациюОбОшибкеВПротокол(26, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()), Объект, КомпонентыОбмена, Тип);
...
//  КомпонентыОбмена - Структура - содержит все правила и параметры обмена.
//
Процедура ЗаписьЖурналаРегистрацииОбменДанными(Комментарий, КомпонентыОбмена, Уровень = Неопределено) Экспорт
...
Функция ТаблицаПравилКонвертации(XMLСхема, МенеджерОбмена , НаправлениеОбмена, ПравилаОбработкиДанных)
	...
	Если НаправлениеОбмена = "Получение" Тогда
		...
		// Для найденых строк необходимо заполнить реквизит "РазрешитьСоздаватьОбъектИзСтруктуры".
		...
		// тогда реквзит заполняется значением "Истина", 
...
Функция СсылкаОбъектаПоСвойствамОбъектаXDTO(ПравилоКонвертации, ПолученныеДанные)
	...
	Для Каждого ПопыткаПоиска Из ПравилоКонвертации.ПоляПоиска Цикл
		...
		// В поиске участвуют только заполненные поля.
		...
		Для Каждого ПолеПоиска Из ПоляПоиска Цикл
			Если Не ЗначениеЗаполнено(ПолеПоиска.Значение) Тогда
				НезаполненныеПоля.Добавить(ПолеПоиска.Ключ);
			КонецЕсли;
		КонецЦикла;
		
		Для Каждого НезаполненноеПоле Из НезаполненныеПоля Цикл
			ПоляПоиска.Удалить(НезаполненноеПоле);
		КонецЦикла;
...
Процедура ОтложенноеЗаполнениеОбъектов(КомпонентыОбмена)
	...
	Для Каждого СтрокаТаблицы Из ЗагруженныеОбъекты Цикл
		...
		Исключение
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка выполнения обработчика ПослеКонвертацииВсехДанных.
				|Тип источника: %1
				|Описание ошибки: %2'"),
				Строка(Объект),
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если ЗначениеЗаполнено(ПОД) Тогда
		...
		Для Каждого ПравилоКонвертацииИмя Из МассивИменПКО Цикл
			...
			Если ПравилоКонвертации.ВариантИдентификации = "СначалаПоУникальномуИдентификаторуПотомПоПолямПоиска"
				...
				МассивОбъектовКУдалению.Добавить(СсылкаОбъектаПоУИДОбъектаXDTO(УникальныйИдентификаторСтрокой, ПравилоКонвертации.ТипДанных, КомпонентыОбмена));
			КонецЕсли;
		КонецЦикла;
	...
	Для Каждого ЭлементКУдалению Из МассивОбъектовКУдалению Цикл
		...
		Если КомпонентыОбмена.РежимЗагрузкиДанныхВИнформационнуюБазу Тогда
			Если Метаданные.Документы.Содержит(Объект.Метаданные()) И Объект.Проведен Тогда
				ОтменитьПроведениеОбъектаВИБ(Объект, КомпонентыОбмена.УзелКорреспондента);
			КонецЕсли;
			ОбменДаннымиСервер.УстановитьОбменДаннымиЗагрузка(Объект, Истина, Ложь, КомпонентыОбмена.УзелКорреспондента);
...
Процедура УдалитьДвиженияУДокумента(ДокументОбъект, Отказ)
	...
	Для Каждого СтрокаДвижения Из ТаблицаДвижений Цикл
		...
		Если НЕ ПравоДоступа("Изменение", Набор.Метаданные()) Тогда
			...
			ВызватьИсключение НСтр("ru = 'Нарушение прав доступа:'") + " " + СтрокаДвижения.Имя;
			Возврат;
		КонецЕсли;
	...
	Для Каждого СтрокаДвижения Из МассивОбрабатываемыхСтрокТаблицыДвижений Цикл		
		...
		Исключение
			...
			ВызватьИсключение НСтр("ru = 'Операция не выполнена:'") + " " + СтрокаДвижения.Имя
...
Процедура ЗаписатьПриНеобходимостиПубличныйИдентификатор(ДанныеДляЗаписиВИБ, ПолученныеДанныеСсылка, УзелОбмена, ПравилоКонвертации)
	
	ВариантИдентификации = СокрЛП(ПравилоКонвертации.ВариантИдентификации);
	Если Не ВариантИдентификации = "СначалаПоУникальномуИдентификаторуПотомПоПолямПоиска" Тогда
		Возврат;
	КонецЕсли;
	...
	Если ПолученныеДанныеСсылка = ДанныеДляЗаписиВИБ.Ссылка Тогда
		Возврат;
	КонецЕсли;
		
	СтруктураЗаписи = Новый Структура;
	СтруктураЗаписи.Вставить("Идентификатор", ПолученныеДанныеСсылка.УникальныйИдентификатор());
	СтруктураЗаписи.Вставить("Ссылка", ДанныеДляЗаписиВИБ.Ссылка);
