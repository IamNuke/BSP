...
Процедура ПроверитьКонтрагентовВДокументеФоновоеЗадание(Форма, ПараметрыФоновогоЗадания) Экспорт
	...
	ВыполнитьПредварительнуюПроверкуКонтрагентовВДокументе(Форма, ПараметрыФоновогоЗадания);
...
Процедура ПриСозданииНаСервереДокумент(Форма, Параметры) Экспорт
	...
	Если Форма.РеквизитыПроверкиКонтрагентов.ПроверкаИспользуется Тогда 
		...
		Если НЕ ЭтоМедленныйРежимРаботы() Тогда
			...
			ПараметрыФоновогоЗадания = ПроверкаКонтрагентовКлиентСервер.ПараметрыФоновогоЗадания(Ложь);
			ВыполнитьПредварительнуюПроверкуКонтрагентовВДокументе(Форма, ПараметрыФоновогоЗадания);
		КонецЕсли;
...
Процедура ИнициализироватьРеквизитыФормыДокумент(Форма) Экспорт
	...
	// Служебный реквизит
	НовыеРеквизиты.Вставить("ЭтоПроверкаКонтрагентовПриОткрытии", 	Истина);
...
// Устанавливает текст и видимость предупреждения про медленную работу.
Процедура УстановитьВидимостьИЗаголовокПредупрежденияПроМедленнуюРаботу(Элемент) Экспорт
	
	Если ЭтоМедленныйРежимРаботы() Тогда
		Элемент.Видимость = Истина;
		Элемент.Заголовок = 
			НСтр("ru = 'Включение сервиса может замедлить открытие документов.'");
	Иначе
		Элемент.Видимость = Ложь;
	КонецЕсли;
	...
	Иначе
		...
		Если СтандартнаяОбработка Тогда
			...
			Для каждого ДанныеКонтрагента Из ДанныеКонтрагентов Цикл
				...
				Если НЕ ЕстьКолонкаЭтоЮридическоеЛицо И НЕ ЗначениеЗаполнено(ДанныеКонтрагента.ЭтоЮридическоеЛицо) Тогда
					ДанныеКонтрагента.ЭтоЮридическоеЛицо = ЭтоЮридическоеЛицо(ДанныеКонтрагента);
				КонецЕсли;
				
				// Заполняем только пустые значения
				Если НЕ ЕстьКолонкаЭтоИностранныйКонтрагент И НЕ ЗначениеЗаполнено(ДанныеКонтрагента.ЭтоИностранныйКонтрагент) Тогда
...
Процедура ВзятьСостоянияИзКэшаПриПроверкеСправочника(ДанныеКонтрагентов, ДополнительныеПараметрыПроверки = Неопределено)
	...
	Если ДополнительныеПараметрыПроверки <> Неопределено
		...
		Если ЭтоПроверкаСправочника
			...
			ИЛИ ЭтоСмешанныйРежимПолученияСостояний Тогда
...
Процедура ПолучитьСостоянияКонтрагентовИзКэша(ДанныеКонтрагентов, ПолучатьВсеСостоянияИзРегистра = Ложь)
	...
	Запрос.УстановитьПараметр("СостоянияСОшибками", 			ПроверкаКонтрагентовКлиентСерверПовтИсп.СостоянияКонтрагентаСОшибкой());
	Запрос.УстановитьПараметр("ДанныеКонтрагентов", 			ДанныеКонтрагентов);
...
Процедура ВыполнитьПредварительнуюПроверкуКонтрагентовВДокументе(Форма, ПараметрыФоновогоЗадания)
	
	ОчиститьРезультатыПредыдущихПроверок(Форма, ПараметрыФоновогоЗадания);
	ПоказатьПредварительныйРезультатПроверкиКонтрагентовИзКэшаВДокументе(Форма, ПараметрыФоновогоЗадания);
	
КонецПроцедуры

Процедура ПоказатьПредварительныйРезультатПроверкиКонтрагентовИзКэшаВДокументе(Форма, ПараметрыФоновогоЗадания)
	...
	Форма.РеквизитыПроверкиКонтрагентов.АдресДанныхКонтрагентовИзКэша = ПоместитьВоВременноеХранилище(ДанныеКонтрагентов, Новый УникальныйИдентификатор);
	Форма.РеквизитыПроверкиКонтрагентов.АдресХранилища = Неопределено;
