...
Функция Подключаемый_ПроверитьПравильностьУсловныхВызовов()
	...
	Для Каждого Файл Из МассивМодулей Цикл
		...
		ОпределитьОшибкиУсловныхВызовов(Файл.Имя, СтрокаТекста, СписокОшибочныхОбъектов, СписокИспользуемыхПодсистем);
	КонецЦикла;
...
Функция ИмяПодсистемыЗаданоНекорректно(УсловныйВызов, СписокИспользуемыхПодсистем)
	...
	Если СтрНайти(УсловныйВызов, "ТехнологияСервиса.") > 0
		Или СтрНайти(УсловныйВызов, "ИнтернетПоддержкаПользователей") > 0
		Или СтрНайти(УсловныйВызов, "ЭлектронноеВзаимодействие") > 0 Тогда
		Возврат Ложь;
	КонецЕсли;
